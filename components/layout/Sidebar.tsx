    ...
    <aside className="w-[220px] flex-shrink-0 flex flex-col bg-[#111118] border-r border-white/[0.07] overflow-hidden">
      ...
      <div className="border-t border-white/[0.07] p-3">
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-[10px] text-[#7a7a9a]">
            <span>{profile?.words_used?.toLocaleString() ?? 0} words used</span>
          ...
          <div className="h-1 bg-[#1c1c28] rounded-full overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-[#6c63ff] to-violet-400 transition-all"
              style={{ width: `${Math.min(usagePct, 100)}%` }}
            />
          </div>
          <div className="text-[10px] text-[#7a7a9a]">
            {profile?.words_limit?.toLocaleString() ?? 5000} word limit
          </div>
        </div>
      </div>
    </aside>
